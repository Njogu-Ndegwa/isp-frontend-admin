        ...
        <div className="flex items-start justify-between p-5 pb-0">
          <div>
            ...
            <p className="text-xs text-foreground-muted mt-0.5">
              {step === 'window' && 'Pick the downtime window; customers with an active package get it back.'}
              ...
              {preview.skipped_expired.length > 0 && (
                <details className="border border-border rounded-xl px-3 py-2">
                  <summary className="text-xs text-foreground-muted cursor-pointer select-none">
                    {preview.skipped_expired.length} already-expired customer{preview.skipped_expired.length === 1 ? '' : 's'} not reactivated
                  </summary>
          ...
          {step === 'done' && result && (
            <div className="text-center py-6">
              ...
              <p className="text-sm text-foreground-muted">
                {formatDuration(result.total_seconds_credited)} of lost time given back to active packages.
              </p>
