  ...
  return (
    <div className="space-y-5 pb-24 md:pb-6">
      ...
      <div>
        ...
        <h2 className="text-lg font-semibold text-foreground">{t('Payment History')}</h2>
        <p className="text-xs text-foreground-muted mt-0.5">{t('Your subscription payment records')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl bg-background-secondary border border-border p-8 text-center">
          ...
          <button onClick={fetchPayments} className="btn-primary px-4 py-2 text-sm">{t('Retry')}</button>
        </div>
      ) : payments.length === 0 ? (
        <div className="rounded-2xl bg-background-secondary border border-border p-8 text-center">
          ...
          <p className="text-sm text-foreground-muted">{t('No payments yet')}</p>
        </div>
      ) : (
        <>
          {/* Desktop Table */}
          <div className="hidden md:block">
            <DataTable
              ...
              emptyState={{ message: t('No payments found') }}
            />
          </div>
          ...
          <div className="md:hidden space-y-2">
            {payments.map((p) => (
              <MobileDataCard
                ...
                status={{ label: t(p.status), variant: p.status === 'completed' ? 'success' : p.status === 'pending' ? 'warning' : 'danger' }}
                fields={[
                  { value: formatSafeDate(p.created_at) },
                  ...(p.invoice_id ? [{ value: t('Invoice #{id}', { id: p.invoice_id }) }] : []),
                ]}
                layout="compact"
              />
            ))}
