// Country dial-code data for the phone-number selector.
// Single source of truth — no external dependency.
// `iso2` is the uppercase ISO 3166-1 alpha-2 code; `dialCode` excludes the leading "+".
...
// Alphabetical by name. Default selection (Kenya) is chosen by code, not by position.
...
export const DEFAULT_COUNTRY_ISO2 = 'KE';

export const DEFAULT_COUNTRY: Country =
  COUNTRIES.find((c) => c.iso2 === DEFAULT_COUNTRY_ISO2) ?? COUNTRIES[0];

/**
 * Returns the emoji flag for an ISO 3166-1 alpha-2 code by mapping each letter
 * to its regional-indicator symbol. On platforms whose default fonts lack flag
 * glyphs (e.g. Chrome on Windows) this degrades to the two-letter code, which
