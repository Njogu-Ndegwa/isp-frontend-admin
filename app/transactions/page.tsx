                ...
                filteredTransactions.map((tx, index) => (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-full bg-accent-secondary/10 flex items-center justify-center text-accent-secondary font-medium text-sm flex-shrink-0">
                        {tx.customer?.name.charAt(0).toUpperCase() || '?'}
                      </div>
                      <div className="min-w-0">
                        <p className="font-medium text-foreground truncate">{tx.customer?.name || '-'}</p>
                        <p className="text-xs text-foreground-muted font-mono">{tx.phone_number}</p>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0 ml-3">
                      <p className="font-semibold text-accent-primary">KES {tx.amount.toLocaleString()}</p>
                      <span className={`badge ${getStatusBadge(tx.status)} capitalize text-[11px]`}>{tx.status}</span>
                    </div>
                  </div>

                  {/* Middle row: Plan + Date + ID */}
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-foreground">{tx.plan?.name || '-'}</span>
                      {tx.plan && (
                        <span className="text-xs text-foreground-muted">
                          {tx.plan.duration_value} {tx.plan.duration_unit}
                        </span>
                      )}
                    </div>
