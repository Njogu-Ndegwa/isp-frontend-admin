...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Download Usage Over Time */}
      {selectedRouterId && (
        <DownloadUsageSection
          data={bandwidth}
...
function formatUsageMb(mb: number) {
...
function DownloadUsageSection({
  ...
  onServiceChange: (service: DownloadUsageServiceFilter) => void;
}) {
      ...
      <div className="card p-5 border-red-500/30 animate-fade-in">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-foreground">Download Usage Over Time</p>
              <p className="text-xs text-red-400">{error}</p>
            </div>
          </div>
          <button onClick={onRetry} className="btn-ghost text-xs">
    ...
    return (
      <div className="card p-5 animate-fade-in">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl skeleton" />
          <div>
            <div className="w-44 h-4 skeleton mb-1" />
    ...
    <div className="card p-4 sm:p-5 animate-fade-in">
      <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-4 mb-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <p className="font-semibold text-foreground text-sm sm:text-base">Download Usage Over Time</p>
              {loading && <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />}
            </div>
            <p className="text-[10px] sm:text-xs text-foreground-muted">
              Last {data?.periodHours ?? hours}h - {data?.count ?? 0} points
            </p>
          </div>
        </div>
