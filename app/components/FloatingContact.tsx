...
export default function FloatingContact({ whatsappMessage = WHATSAPP_DEFAULT_MESSAGE }: { whatsappMessage?: string }) {
  ...
  return (
    <div
      aria-hidden={hidden}
      className={`fixed right-4 bottom-4 z-40 flex flex-col items-end gap-3 pb-[env(safe-area-inset-bottom)] transition-opacity duration-200 ${
    ...
    >
      ...
      <a
        ...
        className="h-14 pl-4 pr-5 rounded-full bg-background/95 backdrop-blur border border-amber-500/50 text-amber-500 font-semibold text-sm shadow-xl flex items-center gap-2 hover:bg-amber-500/10 active:scale-95 transition-all"
      >
      ...
      <a
        ...
        className="h-14 pl-4 pr-5 rounded-full bg-[#25D366] text-[#062e18] font-semibold text-sm shadow-xl shadow-emerald-900/20 flex items-center gap-2 hover:brightness-105 active:scale-95 transition-all"
      >
