...
export default function AdDetailPage() {
  ...
  const [clicksPerPage, setClicksPerPage] = useState(20);
  const [impressionsPage, setImpressionsPage] = useState(1);
  const [impressionsPerPage, setImpressionsPerPage] = useState(20);
  ...
  }, [activeTab, clicksPage, clicksPerPage, impressionsPage, impressionsPerPage, clickTypeFilter]);
  ...
  const loadImpressions = async () => {
    try {
      const response: AdImpressionsResponse = await api.getAdImpressions(adId, impressionsPage, impressionsPerPage);
    ...
    <div>
      ...
      {activeTab === 'clicks' && (
        <div className="animate-fade-in">
          ...
          <Pagination
            page={clicksPage}
            perPage={clicksPerPage}
            total={clicksTotal}
            onPageChange={setClicksPage}
            onPerPageChange={(pp) => { setClicksPerPage(pp); setClicksPage(1); }}
            loading={loading}
            noun="clicks"
          />
        </div>
      ...
      {activeTab === 'impressions' && (
        <div className="animate-fade-in">
          ...
          <Pagination
            ...
            perPage={impressionsPerPage}
            total={impressionsTotal}
            onPageChange={setImpressionsPage}
            onPerPageChange={(pp) => { setImpressionsPerPage(pp); setImpressionsPage(1); }}
            loading={loading}
            noun="impressions"
          />
        </div>
